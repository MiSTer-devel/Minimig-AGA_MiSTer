...
// it checks if predefined period of time has elapsed and scrolls the name if necessary
    ...
    if (DirEntryLFN[(int)k][0] && CheckTimer(scroll_timer)) // scroll if long name and timer delay elapsed
    {
        ...
        len = strlen(DirEntryLFN[(int)k]); // get name length

        if (len > 4)
            if (DirEntryLFN[(int)k][len - 4] == '.')
                len -= 4; // remove extension

        if (len > 30) // scroll name if longer than display size
        {
            if ((long)scroll_offset >= (len + BLANKSPACE) << 2) // reset scroll position if it exceeds predefined maximum
                scroll_offset = 0;
            ...
            if (DirEntry[(int)k].Attributes & ATTR_DIRECTORY)
            ...
            if (len > 0)
                strncpy(s, &DirEntryLFN[(int)k][offset], len); // copy name substring

            if (len < max_len - BLANKSPACE) // file name substring and blank space is shorter than display line size
                strncpy(s + len + BLANKSPACE, &DirEntryLFN[(int)k][0], max_len - len - BLANKSPACE); // repeat the name after its end and predefined number of blank space
...
// comparision is case sensitive
    ...
    if (len > 20) // scan only names which can't be fully displayed
    {
        for (i = (unsigned short)len - 1 - sizeof(template); i > 0; i--) // scan through the file name starting from its end
        {
            ...
            ptr2 = template;
            cmp = 0;
            for (k = 0; k < (short)sizeof(template); k++) // scan through template
            {
