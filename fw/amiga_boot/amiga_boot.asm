...
;	dc.b	"20100903"	; FPGA core version - 8 ASCII characters
;------------------------------------------------------------------------------
Start:
;------------------------------------------------------------------------------
;	lea	$010000,A7	; 
...
;-------------------------------
	...
	move.l	(A0)+,D0    ; move *(A0) -> D0
	bsr	DiskRead        ; branch to subroutine
	lea	disk_buffer,A0  ; load effective address &(disk_buffer) -> A0
	cmp.b	#$FE,(A0)     ; compare: *(A0) - $FE
	bne.s	no_red        ; branch if not equal (no zero flag)
	move.w	#$0F00,COLOR0(A6)	;red screen
	addq	#1,A0         ; add quick: add one to A0
no_red:
	bsr	PrintText       ; branch to subroutine

	moveq	#10,D0        ; move quick: move 10 to D0
	bsr	putc            ; branch to subroutine

	bra	end_cmd         ; branch always
...
;	dc.b	$C0,$60,$30,$18,$0C,$06,$03,$00	; Backslash
...
;	dc.b	10,"Incompatible Menue firmware!",0
...
;	dc.b	"Bug fixes, mods and extensions by Jakub Bednarski and Sascha Boing ",10
;	dc.b	"For updates and support please visit www.minimig.net",10
;	dc.b	"TG68K.C (68000 IP Core) and Chameleon Port by Tobias Gubener",10,0

bootloader_msg:
;	dc.b	10,"Bootloader    2010-09-10",0

fpga_msg:
;	dc.b	10,"Minimig core  2011-04-10",0
