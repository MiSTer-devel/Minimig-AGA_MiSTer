 ...
 * original a314 version by Niklas Ekström
 */
...
#define UMKBADDR(x) (((ULONG)x) >> 2)

#define ID_DISK 0x536F7267
...
char default_volume_name[] = "\006MiSTer";

char device_name[32];

// These are hardcoded shared memory
unsigned short *request_flg = (unsigned short *)0xDD4000; // 4B
char *request_buffer        = (char *)0xDD4004; // 512B
char *data_buffer           = (char *)0xDD5000; // 4KB
...
void dbg_init()
{
	dbg_log = (struct FileHandle *)BADDR(Open("CON:200/0/440/256/MiSTer_fs", MODE_NEWFILE));
...
void create_and_add_volume()
{
	...
	my_volume->dl_Name = (BSTR)UMKBADDR(default_volume_name);
	my_volume->dl_Type = DLT_VOLUME;
	my_volume->dl_Task = mp;
	my_volume->dl_DiskType = ID_DISK;
	...
	info->di_DevInfo = UMKBADDR(my_volume);
	Permit();
}
...
{
	*(int*)request_buffer = len;
	unsigned short *p = request_flg;
	p[1] = 0xA55A;
	unsigned short n = p[0] + 1;
	n = ((n + 77) << 8) | (n & 0xFF);
	*p++ = n;
	while(n != *p){};
}
...
struct FileLock *create_and_add_file_lock(long key, long mode)
{
	...
	lock->fl_Access = mode;
	lock->fl_Task = mp;
	lock->fl_Volume = UMKBADDR(my_volume);

	Forbid();
	lock->fl_Link = my_volume->dl_Lock;
	my_volume->dl_Lock = UMKBADDR(lock);
	Permit();
...
void action_locate_object(struct DosPacket *dp)
{
	...
	dbg("  parent lock = $l, mode = $s\n", parent, mode == SHARED_LOCK ? "SHARED_LOCK" : "EXCLUSIVE_LOCK");
	dbg("  name = $S\n", name);
	...
	else
	{
		...
		dbg("  Returning lock ($l) $l\n", res->key, lock);
		dp->dp_Res1 = UMKBADDR(lock);
		dp->dp_Res2 = 0;
	}
...
void action_examine_next(struct DosPacket *dp)
{
	...
	{
		dbg(" END\n");
		if(res->error_code != ERROR_NO_MORE_ENTRIES)
		{
			dbg("  Failed, error code $l\n", (LONG)res->error_code);
		}
		dp->dp_Res1 = DOSFALSE;
...
void fill_info_data(struct DosPacket *dp, struct InfoData *id)
{
	...
	id->id_VolumeNode = UMKBADDR(my_volume);
	id->id_InUse = DOSTRUE;
}
